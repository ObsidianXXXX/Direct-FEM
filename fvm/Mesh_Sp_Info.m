function [meshInfo,spInfo] = Mesh_Sp_Info(l,h,nelx,nely,nspx,nspy)
    ...
    xsp = linspace(0,l,spx);
    ysp = linspace(0,h,spy);
    
    [xxsp,yysp] = meshgrid(ysp,xsp);
    spcoord(:,1) = xxsp(:); spcoord(:,2) = yysp(:);
    X_sp = spcoord(:,2); Y_sp = spcoord(:,1);
    % nspx=nspy=3
    % strainlist = [1 3 5;15 17 19;29 31 33]
    strainlist = (nspy - 1) * spx * (0:(nely - 1))'+ (1:(nspx - 1):(spx - nspx + 1));
    % nspx=nspy=3
    % IDlist=[0 1 2;7 8 9;14 15 16]
    IDlist = ((0:(nspy - 1))' * spx + (0:(nspx - 1)));
    eleSpID = reshape(strainlist',nEl,1) + reshape(IDlist',1,spnEl);
    % X direction first
    spInfo.spnEl = spnEl; spInfo.eleSpID = eleSpID;
    spInfo.nstrain = nstrain;
    spInfo.plot_x = X_sp; spInfo.plot_y = Y_sp;
    spInfo.nspx = nspx; spInfo.nspy = nspy;
    spInfo.spxEl = X_sp(eleSpID); spInfo.spyEl = Y_sp(eleSpID); 
end